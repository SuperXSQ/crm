...
<%@ page import="cn.xsq.crm.settings.domain.DicValue" %>
<%@ page import="java.util.Set" %>
<%@ page import="cn.xsq.crm.workbench.domain.Tran" %>
...
String basePath = request.getScheme() + "://" + request.getServerName() + ":" + 	request.getServerPort() + request.getContextPath() + "/";
    ...
    //根据pMap 取得key集合（阶段stage）
    Set<String> set = pMap.keySet();

    //取得：前面正常阶段 和 后面丢失阶段的 分界点下标
    //根据可能性来判断  遍历，如果可能性为0，则为后面丢失阶段
    int point = 0;
...
<script type="text/javascript">
    ...
    function changeIcon(stage, i) {
        ...
        //当前阶段的可能性 点击图标之后页面的可能性更新  直接从页面取得
        var possibility = $("#possibility").html();
        //从脚本取得分界点
        var point = "<%=point%>";
        //当前阶段的下标（为了与for循环中的i区分）
        var index = i;
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
        ...
        <%
            ...
            int currentIndex = 0;
            for (int i=0; i<stageList.size(); i++){
                String tStage = stageList.get(i).getValue();
                if (tStage.equals(currentStage)){
                    currentIndex = i;
                    break;
                }
            }
            ...
            //如果当前的可能性为0 说明当前处于丢失阶段（最后两个阶段） 前面7个黑圈  后两个一个红×一个黑×
            if ("0".equals(currentPossibility)){
                ...
                for (int i=0; i<stageList.size(); i++){
                    ...
                    String tempStage = dv.getValue();
                    //不需要
                    //当前遍历阶段的可能性是
                    String tempPossibility = pMap.get(tempStage);

                    //当前遍历的阶段的下标 大于 分界点，说明是 x
                    //if (i >= point){
                    if ("0".equals(tempPossibility)){
                        ...
                        if (currentStage.equals(tempStage)){
                            //-------------------------红x
                            %>
        <span id="<%=i%>" onclick="changeStage('<%=tempStage%>','<%=i%>')"
        ...
        -----------
                        <%
                    //当前遍历的阶段的下标 小于 分界点，说明可能是 绿圈 绿色标记 黑圈
                    }else {
                        //遍历出来的阶段下标  小于 当前阶段的下标  说明是绿圈
                        if (i<currentIndex){
                            //-------------------------绿圈
                            %>
        <span id="<%=i%>" onclick="changeStage('<%=tempStage%>','<%=i%>')"
